...
```bash
# Make sure a Twenty server is running at the configured TWENTY_API_URL
yarn test
```

Test configuration is loaded from `.env.test` at the project root:

- `TWENTY_API_URL` - URL of the Twenty server (default: `http://localhost:3000`)
- `TWENTY_TEST_API_KEY` - API key for authenticating with the Twenty instance

The `.env.test` file is committed with defaults for a local dev seed instance. In CI, these values can be injected as environment variables which take precedence over the file.
